...
# --- LEFT HALF: real padlock image ---
base = Image.open('/home/fuzzy/IMG_0707.png').convert('RGB')
bw, bh = base.size
scale = max(580 / bw, H / bh)
base = base.resize((int(bw * scale), int(bh * scale)), Image.LANCZOS)
bw2, bh2 = base.size
left = base.crop(((bw2 - 580) // 2, (bh2 - H) // 2, (bw2 - 580) // 2 + 580, (bh2 - H) // 2 + H))
left = ImageEnhance.Brightness(left).enhance(1.15)
left = ImageEnhance.Color(left).enhance(1.3)
